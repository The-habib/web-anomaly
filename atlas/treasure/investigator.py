...
Strictly prohibits automatic machine validation.
"""
...
def investigate_single_candidate(
    ...
    """
    Deeply investigate a single candidate URL and collect verifiable evidence.
    """
    now_utc = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    study_id = f"invest_{cand.candidate_id}_{cand.domain}"

    # 1. Fetch Live Deep Evidence & Freeze Artifact
    ...
    elif deep_ev.live_status_code == 0:
        survival = SurvivalState.UNKNOWN
    else:
        survival = SurvivalState.ARCHIVED_ONLY

    # 5. Compute Treasure Quality Score (0 - 100) strictly for ranking
    score = 20.0  # base
    if deep_ev.has_tables_layout:
        score += 25.0
    if deep_ev.has_retro_elements:
    ...
    if len(cand.seen_by_strategies) >= 3:
        score += 10.0
    if cand.earliest_capture_year and cand.earliest_capture_year <= 2000:
        score += 15.0
    if deep_ev.frameworks_detected:
        score -= 30.0
    ...
    # 7. Prior Art Classification (Generic obscurity based on signals)
    if len(features) >= 3 and "orphaned_from_root_navigation" in features:
        prior_art = PriorArtStatus.OBSCURE
    elif score >= 45.0:
        prior_art = PriorArtStatus.POORLY_DOCUMENTED
    else:
        prior_art = PriorArtStatus.DOCUMENTED

    # 8. Decision Classification (STRICT MACHINE PROMOTION PROHIBITION)
    # High score nominiates to REVIEW_PENDING or POTENTIAL_TREASURE.
    # ONLY genuine human review can promote to TREASURE_VALIDATED.
    ...
    if deep_ev.has_tables_layout:
        explanation += "The page preserves table-based HTML layout and vintage formatting. "
    if not is_linked:
        explanation += "It is orphaned from modern homepage navigation. "

    why_int = (
        f"This page survives in active service ({deep_ev.live_status_code} OK) on {cand.domain}. "
        f"It exhibits {len(features)} structural archaeological signals without modern frontend frameworks."
    )

    why_miss = (
        f"Modern web crawlers and commercial search miss this surface because it is {path_depth} directories deep, "
        f"lacks incoming links from the current root homepage, and exhibits legacy structural layout."
    )

    timeline_str = "Archival continuity verified."
    if cand.earliest_capture_year and cand.latest_capture_year:
        timeline_str = f"Historical archive span: {cand.earliest_capture_year}–{cand.latest_capture_year} ({cand.historical_span_years} years)."
...
def run_adaptive_investigations(
    ...
    nominated_candidates: List[InvestigationRecord] = []
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_cand):
            ...
            if inv_res.decision in (TreasureDecision.REVIEW_PENDING, TreasureDecision.POTENTIAL_TREASURE):
                nominated_candidates.append(inv_res)
                print(f"    [+] Nominated Candidate for Review: {inv_res.domain}{inv_res.path} (Score: {inv_res.treasure_score:.1f}, {inv_res.discovery_difficulty.value})")

            if completed % 20 == 0 or completed == len(investigation_pool):
                print(f"    Progress: {completed}/{len(investigation_pool)} investigated ({len(nominated_candidates)} review-pending nominations)...")
    ...
    print(f"[+] Investigations complete: {len(all_investigations)} investigated, {len(nominated_candidates)} nominated for review.")
    return all_investigations, nominated_candidates
